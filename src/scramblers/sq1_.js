...
var scrambler = (function () {
  ...
  function FullCube_randomCube() {
    ...
    shape = Shape_ShapeIdx[~~(Math.floor(square1SolverRandomSource.random() * 3678))];
    corner = 0x01234567 << 1 | 0x11111111;
    edge = 0x01234567 << 1;
    n_corner = n_edge = 8;
    for (i = 0; i < 24; i++) {
      if (((shape >> i) & 1) == 0) {//edge
        rnd = ~~(Math.floor(square1SolverRandomSource.random() * n_edge)) << 2;
        FullCube_setPiece(f, 23 - i, (edge >> rnd) & 0xf);
        m = (1 << rnd) - 1;
        edge = (edge & m) + ((edge >> 4) & ~m);
        --n_edge;
      } else {//corner
        rnd = ~~(Math.floor(square1SolverRandomSource.random() * n_corner)) << 2;
    ...
    f.ml = ~~(Math.floor(square1SolverRandomSource.random() * 2));
    //	console.log(f);
    return f;
  }
