...
export class MinxScrambler implements Scrambler {
  ...
  getRandomScramble() {
    const seq = new Array();
    for (let i = 0; i < this.linenbr * this.linelen; i++) {
      seq[i] = Math.floor(this.randomSource.random() * 2);
    }
    ...
    for (let j = 0; j < this.linenbr; j++) {
      for (let i = 0; i < this.linelen; i++) {
        if (i % 2) {
          if (seq[j * this.linelen + i]) {
            s += "D++ ";
            state = this.applyMove(state, this.permD2);
          }
          else {
            s += "D-- ";
            state = this.applyMove(state, this.permD2i);
          }
        }
        else {
          if (seq[j * this.linelen + i]) {
            s += "R++ ";
            state = this.applyMove(state, this.permR2);
          }
          else {
            s += "R-- ";
            state = this.applyMove(state, this.permR2i);
          }
        }
      }
      if (seq[(j + 1) * this.linelen - 1]) {
        s += "U";
        state = this.applyMove(state, this.permU);
      }
      else {
        s += "U'";
        state = this.applyMove(state, this.permUi);
      }
      if (j < this.linenbr - 1) {
        s += "\n";
      }
  ...
  private applyMove(state: any, movePerm: any) {
    let stateNew = [];
    for (let i = 0; i < 11 * 12; i++) {
